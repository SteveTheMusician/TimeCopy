...
import { AmagProTimeDetectionContent } from "../../../dlc/platforms/AmagProTime/detection/AmagProTime.detection.js"
...
export function detectionItem(detectionItems) {
  ...
  function createDetectionItemContent(detectionItem) {
    let detectionContent = ''
    if(detectionItem.bookingsheet === 'AmagProTime') {
      detectionContent = AmagProTimeDetectionContent (detectionItem)
    }
  ...
  if (detectionItems) {
    document.getElementById('window_detection').innerHTML = detectionItems.map(detectionItem =>
      `<div class="configItem detectionItem `+(detectionItem.bookingsheet? '': 'dFlex')+`" name="item_detection" id="` + detectionItem.id + `">
        <div class="detectionItem-body dFlex">
              ...
              <div class="configItem-content-row dFlex">
                <div class="detectionItem-content-larger">
                  <select id="select_bookingPlatform_`+ detectionItem.id + `" title="Weise der Erkennung eine Buchungsplatform zu.">
                    <option value="" selected disabled hidden>Zuordnung wählen</option>`+ (platformSelectItems) +`
                  </select>
                </div>
              </div>
            </div>
          ...
          <div class="detectionItem-dlc dFlex">
            ...
            <div class="detectionItem-dlc-content `+(detectionItem.bookingsheet? '': 'dNone')+`">
              `+(createDetectionItemContent(detectionItem))+`
            </div>
